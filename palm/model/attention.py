...
class PALMAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None):
        try:
            ...
            d_k = self.attention_head_size  # e.g., hidden_size // num_attention_heads
...
class PALMPartialAttention(nn.Module):
    ...
    def forward(self, hidden_states, source_states, source_len, attention_mask=None):
        ...
        # Combine partial mask with the (optional) attention_mask:
        ...
        attention_output = self.dense(context_layer)
        attention_output = self.dropout(attention_output)
        attention_output = self.LayerNorm(attention_output + hidden_states) # Residual connection + LayerNorm
