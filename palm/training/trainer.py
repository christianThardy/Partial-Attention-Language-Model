...
class PALMTrainer:
    ...
    def __init__(self, model, train_dataloader, eval_dataloader, tokenizer, config, device):
        ...
        self.optimizer = AdamW([
            {"params": self.pretrained_params, "lr": constants.PRETRAINED_LEARNING_RATE, "weight_decay": constants.PRETRAINED_WEIGHT_DECAY},
            {"params": self.custom_params, "lr": constants.PALLM_LEARNING_RATE, "weight_decay": constants.CUSTOM_WEIGHT_DECAY, "betas": tuple(constants.CUSTOM_BETAS), "eps": constants.CUSTOM_EPSILON},
        ])
        # Build LR scheduler
        ...
        self.scaler = GradScaler(enabled=(self.device.type == "cuda" and has_fp32_params))
    ...
    def train(self):
        ...
        for epoch in range(constants.NUM_TRAIN_EPOCHS):
            ...
            continuous_unfreeze(self.model, epoch, constants.NUM_TRAIN_EPOCHS, self.NUM_LAYERS)
            ...
            logger.info(f"[Epoch {epoch}/{constants.NUM_TRAIN_EPOCHS}] eval_loss={avg_eval_loss:.4f}, ppl={perplexity:.4f}")
    ...
    def train_epoch(self, epoch):
        ...
        for step, batch in enumerate(tqdm(self.train_dataloader, desc=f"Epoch {epoch+1}")):
            ...
            if (step + 1) % constants.GRADIENT_ACCUMULATION_STEPS == 0 or step == total_steps_in_epoch - 1:
                ...
                # Logging
                if self.global_step % constants.LOG_EVERY_N_STEPS == 0:
                    ...
                    wandb.log({
                        ...
                        "combined_loss": (combined_loss.item() * constants.GRADIENT_ACCUMULATION_STEPS if combined_loss else 0.0),
                    ...
                    logger.info(f"Step {self.global_step}, Loss={loss.item() if loss else 0:.4f}, "
                                f"SAE={sae_loss.item() if sae_loss else 0:.4f}, "
                                f"Comb={combined_loss.item() * constants.GRADIENT_ACCUMULATION_STEPS if combined_loss else 0:.4f}")
